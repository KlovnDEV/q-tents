...
local function open_stash(metadata)
     ...
     TriggerEvent("qb-backpack:client:close", metadata.ID)
end

RegisterNetEvent("qb-backpack:client:open", function(backpack_metadata)
...
local function close_stash(ID)
     QBCore.Functions.TriggerCallback('qb-backpack:server:UpdateWeight', function(weight)
...
RegisterNetEvent("qb-backpack:client:close", function(ID)
...
RegisterNetEvent('qb-backpack:client:enter_password', function(backpack_metadata)
     if backpack_metadata.locked then
          ...
          if inputData then
               ...
               TriggerServerEvent('qb-backpack:server:open_backpack', backpack_metadata)
          end
     else
          TriggerServerEvent('qb-backpack:server:open_backpack', backpack_metadata)
     end
...
RegisterNetEvent('qb-backpack:client:create_password', function(ID)
     local inputData = exports['qb-input']:ShowInput({
          header = 'Password',
          inputs = {
               {
     ...
     if inputData then
          if not inputData.pass then return end
          TriggerServerEvent('qb-backpack:server:add_password', { ID = ID, password = inputData.pass })
     end
end)
